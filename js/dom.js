...
function getFileIcon() {
    // Generic file icon, can be expanded later for specific mime types
    ...
    if (filteredResources.length === 0 && searchQuery) {
        ...
        filteredResources.forEach(resource => {
            ...
            if (resource.type === 'file') {
                resourceContentHTML = `
                    <div class="file-card-content mb-2">
                        ${getFileIcon()}
                        <span class="font-semibold text-sky-400" title="${resource.file.name}">${resource.file.name}</span>
                    </div>
                `;
            } else { // URL
